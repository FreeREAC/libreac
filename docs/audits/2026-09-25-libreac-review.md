...
| M4 broadcast ambiguity | fixed per operator ruling 2026-09-25, HOLD WITH A DECLARED LIMIT: a broadcast sender is held until its own frames prove desk or box, for at most `REAC_DESK_ANNOUNCES_TO_WAIT` (3) x reac-protocol's `ANNOUNCE_PERIOD_MS` (the `timing` group, read through the generated `reac_facts_timing.h`); then, with no master-only frame, it is a box. `reac_sender_kind()` carries the rule; `reac_hunt` (`box_present`, `desk_geometry_live`) and `reac_tap` (an appended `REAC_TAP_STREAM_UNRESOLVED`, resolved by role or by the window) use it; `reac_segment_ident` gains `_kind` answers that publish the caller's verdict. `REAC_HUNT_WINDOW_NS` is the same number, no longer a typed 3e9 | `tests/test_hold.c` (red on both sabotages: an unbounded hold and no hold), `tests/test_tap.c`'s hold and segment arms |
