...
| M4 box width vs desk | fixed per the operator's corrected ruling of 2026-09-25 ("BOX_MAX_CHANNELS = 40 ... S-4000S-3208 (32 in, 8 out), S-2416 (24 in, 16 out), and we tested an 8 in / 32 out box"). A box's width is even per direction, 2..40 (`reac_box_width_ok()`; the limits are reac-protocol's `box_width` facts, read through the generated `reac_facts_box_width.h`), and 40 is legal. The real bug was the rule that 40 wide means the desk. It is replaced by direction, source and role: `reac_rival_kind_of()` (arbitration), the broadcast preference in `reac_detect_rate_fd`, unicast-first in `reac_tap`, and a direction-gated `reac_rx` | `tests/test_desk_or_box.c` (32/8, 24/16, 8/32 and a 40-in box are boxes; the 40-wide broadcast announcing master is the desk; red under the width rule), `tests/test_box_width.c`, `tests/test_rx_twin.c`'s 40+40 arms (red on the old gate) |
| M4 broadcast ambiguity | fixed per operator ruling 2026-09-25, HOLD WITH A DECLARED LIMIT: a broadcast sender is held until its own frames prove desk or box, for at most the SHORTEST master-only control cadence in FRAMES at the current rate — reac-protocol's new `master_cadence` group (FreeREAC/reac-protocol#13: the cfea announce, every 4000 frames of a 48 kHz downstream with or without a box, shorter than page 0x0019's window and the scene repeat; the DECLARED fact is the frame count per rate, 4000 at 48 kHz measured, 3675 / 8000 at 44.1 / 96 kHz INFERRED by the per-rate law; no ms is declared, per the operator's ruling that ms is derived from the rate), read through the generated `reac_facts_master_cadence.h`; then, with no master-only op, it is a box. `reac_sender_kind()` / `reac_master_only_cadence_frames()` / `_ns()` carry it; `reac_hunt` (`box_present`, `desk_geometry_live`) and `reac_tap` (an appended `REAC_TAP_STREAM_UNRESOLVED`, resolved by the source's frames or by its own counter advancing one cadence at its own pace) use it; `reac_segment_ident` gains `_kind` answers. The hunt's separate vacancy window is 3 x `REAC_ANNOUNCE_PERIOD_MS` from the generated `reac_facts_timing.h`, no longer a typed 3e9 | `tests/test_hold.c` (red on both sabotages: an unbounded hold, 7 checks; no hold, 99), `tests/test_tap.c`'s hold arms (the 96 kHz arm reds if the window is pinned to 48 kHz's frames) and segment arms |
| CI hang | fixed: `test_rate_detect`'s feeder blocked in `send()` once the socket buffer (capped at `wmem_max` on a runner) filled. Every test now runs under `tests/run-test.sh` (a timeout on each; exit 77 = explicit SKIP, listed on the verdict line); the netns/veth tests exit 77 when the environment cannot host them | `tests/run-test.sh`; proved on the old hanging binary (exit 124 after the timeout) |
