 ...
 *
 * PURE: one clock and one verdict, no sockets, no frames. The CALLER turns the licence into
 * a reac_hunt verdict and the hunt into a served segment; a wire taken this way KEEPS ITS
 * SNIFFER, because a bet on silence is a bet that has to stay watched — see
 * reac_hunt_silence_proven and the daemon's yield.
 */
#ifndef REAC_KNOCK_H
#define REAC_KNOCK_H
 ...
 *
 * It is also longer than two of the daemon's 200 ms hearing polls, so the silence is
 ...
 * segment, which is the daemon's own watch over the retained sniffer.
 *
